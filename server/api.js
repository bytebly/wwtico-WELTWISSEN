...
// Local verified fallback engine if Gemini API key is not configured or fails
function getVerifiedFallbackResponse(inputMessage) {
  ...
  try {
    ...
    if (apiKey) {
      try {
        ...
        console.warn('Gemini API call failed, falling back to internal verified engine:', geminiError.message);
        const fallback = getVerifiedFallbackResponse(message);
        return res.json(fallback);
      }
    }

    // Default verified response if API key is not supplied
    const fallback = getVerifiedFallbackResponse(message);
    return res.json(fallback);
